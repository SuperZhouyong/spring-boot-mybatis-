...
import com.resumed.sqtwin.configurer.WebMvcConfigurer;
import com.resumed.sqtwin.model.responseban.Bean;
...
public class PublicKetUtils {
    ...
    public static String getPubLicKey() {
        ...
        logger.info("日期" + dateFormat.format(new Date()));
...
//        Log.i("Interceptor", "oldBody=" + oldBody);
     ...
     * @return 判断字符串中有比较符号吗
     */
    public static Bean haveFuhao(String a,String[] arr) {
        for (int i = 0; i < arr.length; i++) {
            if (a.contains(arr[i])) {
                return new Bean(i, arr[i]);
            }
        }
        return new Bean();

    }
    public static Boolean  haveContins(String a,String[] arr) {
        for (int i = 0; i < arr.length; i++) {
            if (a.contains(arr[i])) {
                return true;
            }
